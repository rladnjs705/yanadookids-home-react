...
$(function () {
    ...
    function setGnbPosition() {
        ...
        if (isMob) { // for mobile
            ...
            if (md.os() == 'iOS') {
                try {
                    $('.android_store .btn-app').addClass("store_disable");
                } catch (e) {
                }
                try {
                    $('.android_store2').addClass("disable");
                } catch (e) {
                }
            } else {
                try {
                    $('.ios_store .btn-app').addClass("store_disable");
                } catch (e) {
                }
                try {
                    $('.ios_store2').addClass("disable");
                } catch (e) {
                }
            }

			if($('button.mclose').is(":visible")){
            ...
            //롤링 배너 css수정
            $("#topSliderBtn").removeClass();
            $("#topSliderBtnClose").removeClass();
            $("#topSliderBtn").addClass("m-btn");
            $("#topSliderBtnClose").addClass("m-btn-close");
        } else { // for pc
            ...
            // [START] Store button enable or disable
            try {
                $('.android_store .btn-app').removeClass("store_disable");
            } catch (e) {
            }
            try {
                $('.ios_store .btn-app').removeClass("store_disable");
            } catch (e) {
            }
            mobileMenuReset();
            // [END  ] Store button enable or disable
            ...
            $("#topSliderBtn").removeClass();
            $("#topSliderBtnClose").removeClass();
            $("#topSliderBtn").addClass("btn");
            $("#topSliderBtnClose").addClass("btn-close");
        }
    ...
    function mobileMenuReset(){
		...
		$(".mobile-gnb em").each(function (seq,val) {
			$(val).removeClass('glyphicon-menu-up').addClass('glyphicon-menu-right');
		});
        ...
        swiper = new Swiper('.swiper-container', {
            ...
            on: {
                slideChangeTransitionStart: function (e) {
                    ...
                    _ps.each(function () {
                        TweenMax.fromTo($(this), 1, {x: 0, y: 100, opacity: 0}, {
                            delay: _sec,
                            x: 0,
                            y: 0,
                            opacity: 1,
    ...
    $('#canRefund').click(function (e) {
        ...
        // 환불신청 입력 폼 초기화
        try {
            $("#reundForm #userName").val("");
        } catch (e) {
        }
        try {
            $("#reundForm #userContact").val("");
        } catch (e) {
        }
        try {
            $("#reundForm #userEmail").val("");
        } catch (e) {
        }
        try {
            $("#reundForm #couponType").val("");
        } catch (e) {
        }
        try {
            $("#reundForm #refundReason").val("");
        } catch (e) {
        }

        // 환불신청 입력 폼 에러메시지 초기화
        try {
            $("#refundErrorArea").hide();
        } catch (e) {
        }
        try {
            $("#refundErrorMsg").html("");
...
function displayLoginPage() {
    ...
    try {
        $("#agreeLogin").prop('checked', false);
    } catch (e) {
    }
    try {
        $("#agreeLogin").attr('checked', false);
    } catch (e) {
    }
    try {
...
function colsePolicyPage() {
    if ($("#mobile-mlist").css("display") == "block" || $("#mobile-mclose").css("display") == "block") {
        ...
        $("#modal-policy").hide();
        $(".main-wrapper").show();
        if (modalLoginObj != null && isLoginModelView) {
            openLoginModel();
        }
        if (modalRefundObj != null && isRefundModelView) {
            openRefundModel();
        }
    }
}
